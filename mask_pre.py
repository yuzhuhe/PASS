...
class ProbMask(nn.Module):
    def __init__(self, slope, M, N, scale):
        ...
        self.mult = nn.Parameter(- torch.log(1. / self.para - 1.) / self.slope)

        with torch.no_grad(): #初始化用的
...
class ProbMask_line(nn.Module):
    def __init__(self, slope, M, N, scale):
        ...
        # self.para[int(self.M//2-self.M*self.scale/2):int(self.M//2+self.M*self.scale/2),int(self.N//2-self.N*self.scale/2):int(self.N//2+self.N*self.scale/2), :] = 0
...
class create_mask(nn.Module):
    def __init__(self,M,N,pmask_slope=200.0,sparsity=0.01,sample_slope=200.0,scale=0.1):
