...
def init_model(params):
    ...
    '''
    return model.model(params['M'], params['N'], params['acc_factor'], params['scale'],
...
checkpoint = 'fastMRI_T1_train_8_scale_0.1_iter3/'
prepoint = 'train_pre/'
...
pre_dict = torch.load(prepoint + 'acc8_pre.pth', map_location=torch.device('cpu'))
...
writer = SummaryWriter(comment='PASS_8')
...
for epoch in range(epochs):
    ...
    if (float(psnr2) / number_data2) > 40:
        filtered_state_dict = {k: v for k, v in model.state_dict().items() if 'clip_model' not in k}
        torch.save(filtered_state_dict,
                       checkpoint + 'best_epoch_'+ str(epoch) + '.pth')
