...
def init_model(params):
    ...
    '''
    return model_pre.model(params['M'], params['N'], params['acc_factor'], params['scale'],
...
checkpoint = 'fastMRI_T1_train_pre_8_scale_0.1_iter3/'
...
AD_checkpoint = torch.load(AD_pth, map_location='cpu')
...
params_to_train = [param for name, param in model.named_parameters() if (('clip_model' not in name))]
...
for epoch in range(epochs):
    ...
    with tqdm(total=number_data, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
        ...
        for num_itr in range( number_data//batchSz):
            ...
            # Reconstruction
            _, x_out1, Rec, AD_map = model(org_old, csm)
    ...
    for i in tqdm(range(number_data2)):
        ...
        for aa in loader:
            org_old1, csm1, _ = aa
        _, x_out11, Rec1, AD_map = model(org_old1.to(device=device), csm1.to(device=device))
    ...
    if (float(psnr2) / number_data2) > 40:
        filtered_state_dict = {k: v for k, v in model.state_dict().items() if 'clip_model' not in k}
        torch.save(filtered_state_dict,
